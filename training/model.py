...
class VRCNN(object):
    ...
    def build_recon_model(self,blu,QP):
        ...
        if blu:
            #self.blu_ub = quantization.loadInitBlu(QP)
            self.stepw,self.blu_ub,self.ratio=quantization.loadQpara(QP)
    ...
    def model(self):
        ...
        #self.conv1 = tf.clip_by_value(self.conv1_u, 0, 0.5415, name='blu1')# 3 sigma
        ...
        #self.conv2_1=tf.clip_by_value(self.conv2_1_u, 0, 0.7405, name='blu2_1')
        ...
        #self.conv2_2=tf.clip_by_value(self.conv2_2_u, 0, 0.7405, name='blu2_2')# 3 sigma
        ...
        #self.conv3_1=tf.clip_by_value(self.conv3_1_u, 0, 0.1471, name='blu3_1') #3 sigma
        ...
        #self.conv3_2=tf.clip_by_value(self.conv3_2_u, 0, 0.1471, name='blu3_2')
    ...
    def model_blu(self):
        ...
        self.blu1 = tf.clip_by_value(self.conv1, 0, self.blu_ub[0], name='blu1')
        self.conv2_1 = tf.nn.conv2d(self.blu1, self.weights['w2_1'], strides=[1, 1, 1, 1], padding='SAME') + self.biases['b2_1']
        self.blu2_1=tf.clip_by_value(self.conv2_1,0,self.blu_ub[1],name='blu2_1')
        self.conv2_2 = tf.nn.conv2d(self.blu1, self.weights['w2_2'], strides=[1, 1, 1, 1], padding='SAME') + self.biases['b2_2']
        self.blu2_2=tf.clip_by_value(self.conv2_2,0,self.blu_ub[2],name='blu2_2')
        ...
        self.blu3_1=tf.clip_by_value(self.conv3_1,0,self.blu_ub[3],name='blu3_1')
        self.conv3_2 = tf.nn.conv2d(self.blu2, self.weights['w3_2'], strides=[1, 1, 1, 1], padding='SAME') + self.biases['b3_2']
        self.blu3_2=tf.clip_by_value(self.conv3_2,0,self.blu_ub[4],name='blu3_2')
        ...
        with tf.Session() as sess:
            ...
            for ep in range(config.epoch):
                for idx in range(0, data.pieces//config.batch_size):
                    ...
                    _, err, step = sess.run(fetch_list,feed_dict={self.images: batch[1], self.labels: batch[0]})
                    if step % 10 == 0:
                ...
                if config.blu:
                    self.saver.save(sess,config.blu_dir+str(config.QP)+'/VRCNN.model',global_step=step)
                else:
    ...
    def quant_finetune(self,config):
        ...
        feeddict={}
        update=[]
        stepw = {'w1': self.stepw[0], 'w2_1': self.stepw[1], 'w2_2': self.stepw[2], 'w3_1': self.stepw[3], 'w3_2': self.stepw[4],
                      'w4': self.stepw[5]}
        stepw_b = {'b1': self.stepw[0], 'b2_1': self.stepw[1], 'b2_2': self.stepw[2], 'b3_1': self.stepw[3], 'b3_2': self.stepw[4],
                        'b4': self.stepw[5]}
        ratio = {'b1': self.ratio[0], 'b2_1': self.ratio[1], 'b2_2': self.ratio[2], 'b3_1': self.ratio[3], 'b3_2': self.ratio[4],
                      'b4': self.ratio[5]}
        start_time = time.time()
        with tf.Session() as sess:
            data = train_data.train_data(config.QP, config.batch_size)
            ...
            for w in self.weights:
                wf[w] = sess.run(self.weights[w])
                wq[w]=np.clip(np.around(wf[w]/stepw[w]),-128,127)*stepw[w]
                update.append(tf.assign(self.weights[w],wq[w]))
            ...
            for i in range(0,data.pieces//config.batch_size):
                ...
                sess.run(update)
                _, err, step = sess.run(fetch_list,feed_dict=feeddict)
                if step%10==0:
                ...
                for w in wq:
                    ...
                    wf[w]=np.clip(np.add(wf[w],we[w]),-128*stepw[w],127*stepw[w])
                    wq[w]=np.around(wf[w]/stepw[w])*stepw[w]
                #for b in bq:
            ...
            sess.run(update)
            if config.blu:
                self.saver.save(sess, config.blu_dir + str(config.QP) + '/VRCNN_qfp.model', global_step=step)
            else:
                self.saver.save(sess, config.ppro_dir + str(config.QP) + '/VRCNN_qfp.model', global_step=step)
        return 0
        ...
        with tf.Session() as sess:
            ...
            if config.blu:
                ...
                if self.load(sess, config.ppro_dir + str(config.QP)):
                    ...
                    print(" [!] Load failed...")
                    exit(1)
            #fetch_list = [self.conv1_u, self.conv2_1_u, self.conv2_2_u,self.conv3_1_u, self.conv3_2_u, self.conv4]
            fetch_list = [self.blu1, self.blu2_1, self.blu2_2, self.blu3_1, self.blu3_2, self.conv4]
            batch = data.get_batch(config.batch_size)
    ...
    def conv_validation(self,config):
        ...
        with tf.Session() as sess:
            ...
            for j in range(len(test_data.testData)):
                ...
                #[x, w, b, c] = sess.run([self.images_norm, self.weights['w1'], self.biases['b1'], self.conv1],feed_dict={self.images: input});layer=0
                #[x,w,b,c] = sess.run([self.blu1,self.weights['w2_1'],self.biases['b2_1'],self.conv2_1], feed_dict={self.images: input});layer=1
                #[x, w, b, c] = sess.run([self.blu1, self.weights['w2_2'], self.biases['b2_2'], self.conv2_2],feed_dict={self.images: input});layer=2
                #[x, w, b, c] = sess.run([self.blu2, self.weights['w3_1'], self.biases['b3_1'], self.conv3_1],feed_dict={self.images: input});layer=3
                #[x, w, b, c] = sess.run([self.blu2, self.weights['w3_2'], self.biases['b3_2'], self.conv3_2],feed_dict={self.images: input});layer=4
                [x, w, b, c] = sess.run([self.blu3, self.weights['w4'], self.biases['b4'], self.conv4],feed_dict={self.images: input});layer=5
                print('x:',np.round(x[0,0:5,0:5,0]*self.ratio[layer]))
                print('weights:',np.round(w[0:5,0:5,0,0]/self.stepw[layer]))
                print('biases:',np.round(b[0]/self.stepw[layer]*self.ratio[layer]))
                print('conv:',np.round(c[0,0:5,0:5,0]/self.stepw[layer]*self.ratio[layer]))
                print('conv:',np.round(c[0,0:5,0:5,0]*255))